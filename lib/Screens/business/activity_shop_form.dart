...
class ActivityShopForm extends StatefulWidget {
  @override
  _ActivityShopFormState createState() => _ActivityShopFormState();
}

class _ActivityShopFormState extends State<ActivityShopForm> {
  ...
  final famousFoodItemsController = TextEditingController();
    ...
    if(clicked ==  false) {
      return MaterialButton(
        ...
        onPressed: () async{
          ...
          if(!_formKey.currentState.validate() || shopPhoto == null || passportPhoto == null) {
            ...
            StorageUploadTask uploadThumbnailImage = FirebaseStorage.instance.ref().child("Adventure").
            ...
            final String photo = await imageDownloadUrl.ref.getDownloadURL();

            StorageUploadTask uploadImage = FirebaseStorage.instance.ref().child("Adventure").
            child(userId).child(Timestamp.now().toString()).child('ShopPhoto').putFile(shopPhoto);

            final StorageTaskSnapshot imageDownload =
            (await uploadImage.onComplete);
            final String shop = await imageDownload.ref.getDownloadURL();


            databaseService.addShopToDatabase(state: stateController.text,
                shopAddress: addressController.text,passportPhoto: photo, shopPhoto : shop,type: "Adventure",
                famousThings: famousFoodItemsController.text,email: emailIdController.text,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Add Adventure Activity Business"),
      ),
            ...
            Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 10,),
                  FormInputField(
                    inputLabel: "Famous activities",
                    hintText: "eg: Paragliding, boating,riding",
                    textController: famousFoodItemsController,
                    validator: (value){
                      if(value.isEmpty){
                        return "Field should not be empty";
                      }
